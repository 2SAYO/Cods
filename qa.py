...
# تأكد من أنك قمت بتعيين BOT_TOKEN مسبقًا
...
def _handle_subscriber_long_message(message, text_to_process, selected_voice_details, status_msg_main_scope):
    ...
    try:
        ...
        final_audio_data_helper, ffmpeg_temp_files_helper = convert_and_combine_audio(temp_ogg_files_helper, final_mp3_path_helper)
        all_temp_files_helper.extend(ffmpeg_temp_files_helper)

        if not final_audio_data_helper:
        ...
        caption_text_helper = f"""🎙️ هذه هي رسالتك الطويلة بصوت **{telebot.util.escape(voice_name)}** (تم دمج {total_chunks} أجزاء)

🔄 أرسل /again لإعادة استخدام هذا الصوت مع نص جديد
✨ أرسل /new لاختيار شخصية صوتية مختلفة"""

        logging.info(f"[_handle_subscriber_long_message] Sending final combined MP3 audio ({len(final_audio_data_helper)} bytes) to user {user_id}")
        bot.send_chat_action(chat_id, 	"upload_voice"	)
        try:
            bot.send_audio(chat_id, final_audio_data_helper,
