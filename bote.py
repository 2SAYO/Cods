...
TOKEN = '7803322662:AAFiikXjdzVZ5ncRT3Mw-aWvBj4WwdeqOuc'
...
def start(message):
    ...
    try:
        ...
        if not is_subscribed:
            ...
            channel_button = types.InlineKeyboardButton("Join 💫", url=f"https://t.me/{CHANNEL_ID.strip('@')}")
            markup.add(channel_button)
            
            bot.send_message(message.chat.id, f"🌀 | Oops!\n📢 | Join the channel to use the bot !\n\n🔗 | https://t.me/{CHANNEL_ID.strip('@')}\n\n🚀🚀 | Subscribe, then send /start", reply_markup=markup)
            
            try:
                forward_bot.send_message(
                    ADMIN_ID,
                    f"👤 مستخدم جديد حاول استخدام البوت ولكن يحتاج للاشتراك في القناة:\n\n"
        ...
        bot.send_message(message.chat.id, "⚠️ حدث خطأ أثناء التحقق من الاشتراك. يمكنك المتابعة في الوقت الحالي.")
    ...
    if is_user_upgraded(user_id):
        welcome_message += f"\n\n• 🌟 You are an upgraded with {get_user_max_files(user_id)} file"
    else:
    ...
    try:
        ...
        logger.error(f"Error notifying admin about new/returning user: {e}")
