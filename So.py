...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7001218911:AAF_QZOeYo2wn7Tio47SkE_jPobWRIK6lOQ")
CHANNEL_ID = os.environ.get("CHANNEL_ID", "@Sayo_bots")
ADMIN_ID = int(os.environ.get("ADMIN_ID", 5026029533))
BLOCKED_USERS_FILE = "/home/ubuntu/blocked_users.txt"
UPGRADED_USERS_FILE = "/home/ubuntu/upgraded_users.txt"
USER_DATA_FILE = "/home/ubuntu/user_data.json"
VOICES_PER_PAGE = 15
BANNED_WORDS = ["كس", "منيوج", "كحبة", "عير", "عاهرة", "بلاع", "ينيج"]
MAX_CHARS_FREE = 200
...
try:
    ...
    exit(1) # Exit if bot cannot be initialized
...
FFMPEG_TIMEOUT = 120 # Timeout in seconds for each ffmpeg command

def convert_and_combine_audio(ogg_files, final_mp3_path):
    ...
    temp_files_created = [] # Track all files created/used by this function
    ...
    try:
        if len(ogg_files) == 1:
            ...
            result = subprocess.run(
                ...
                text=True, encoding="utf-8", errors="ignore", # Added errors='ignore'
            ...
            try:
                with open(list_file_path, "w", encoding="utf-8") as f:
                    for ogg_file in ogg_files:
                        ...
                        # Correct escaping for ffmpeg file list: replace ' with '\'', then enclose in single quotes
            ...
            concat_command = [
                ...
                "-i", list_file_path, "-c", "copy", merged_ogg_path
            ]
        ...
        if os.path.exists(final_mp3_path):
            try:
                ...
                final_audio_data = None # Ensure it's None if reading fails
        else:
        ...
        final_audio_data = None # Ensure failure results in None
        ...
        # Cleanup is handled in process_text_input finally block, but we return the list of files
        pass
...
def check_access(func):
    def wrapper(message_or_call):
        try:
            ...
            if user_id != ADMIN_ID:
                try:
                    # Consider caching this check for short periods to reduce API calls
                    member = bot.get_chat_member(CHANNEL_ID, user_id)
                    if member.status not in ['member', 'administrator', 'creator']:
                        raise Exception(f"User status: {member.status}")
                except Exception as e:
                    logging.info(f"Subscription check failed for user {user_id} in channel {CHANNEL_ID}: {e}")
                    markup = InlineKeyboardMarkup()
                    try:
                        channel_username = CHANNEL_ID.lstrip('@')
                        markup.add(InlineKeyboardButton("🔗 الإشتراك في القناة", url=f"https://t.me/{channel_username}"))
                        safe_send_message(chat_id, f"⚠️ عذراً، يجب عليك الاشتراك في قناة البوت أولاً لاستخدامه: {CHANNEL_ID}\n\nاضغط على الزر أدناه للاشتراك ثم أعد المحاولة بالأمر /start.", reply_markup=markup)
                    except Exception as url_err:
                         logging.error(f"Failed to create channel link for {CHANNEL_ID}: {url_err}")
                         safe_send_message(chat_id, f"⚠️ عذراً، يجب عليك الاشتراك في قناة البوت أولاً لاستخدامه: {CHANNEL_ID}\n\nيرجى الاشتراك ثم إعادة المحاولة بالأمر /start.")
...
def display_start_menu(chat_id, user_id, first_name, username, edit_message_id=None):
    """Displays the main menu with user status."""
    ...
    welcome_text = f"""
—————————————————————————•
👋 مرحباً بك: {telebot.util.escape(first_name)}
🆔 الآي دي: `{user_id}`
👤 اليوزر: @{telebot.util.escape(username) if username else 'لا يوجد'}
📊 الحالة: {status}
—————————————————————————•
اختر أحد الخيارات:
"""
...
def display_voice_options(message_or_call, page=1, edit_message_id=None):
    ...
    for i, voice in enumerate(current_voices):
        ...
        # button_text = telebot.util.escape(voice_name)
...
- بعد اختيار الصوت، أرسل النص الذي تريد تحويله.
...
- استخدم أمر /again لإعادة استخدام آخر صوت تم اختياره مع نص جديد.
...
def handle_voice_selection(call):
    ...
    try:
        ...
        prompt_text = f"🎉 رائع! الآن أرسل النص الذي ترغب بتحويله إلى صوت بواسطة شخصية **{telebot.util.escape(voice_name)}**."
...
def process_text_input(message):
    ...
    try:
        ...
        final_audio_data, ffmpeg_temp_files = convert_and_combine_audio(temp_ogg_files, final_mp3_path)
        ...
        audio_filename_display = f"sayo-bot-{random_suffix}.mp3"
        caption_text = f"""🎙️ هذه هي رسالتك بصوت **{telebot.util.escape(voice_name)}**

🔄 أرسل /again لإعادة استخدام هذا الصوت مع نص جديد
✨ أرسل /new لاختيار شخصية صوتية مختلفة"""

        logging.info(f"Sending final MP3 audio ({len(final_audio_data)} bytes) to user {user_id}")
...
def handle_again(message):
    ...
    if user_id in user_last_voice:
        ...
        safe_send_message(chat_id, f"🔄 ستستخدم صوت **{telebot.util.escape(voice_name)}** مرة أخرى.\n📝 أرسل النص الجديد:", parse_mode="Markdown")
        set_user_state(user_id, 'awaiting_tts_text')
    else:
...
def execute_block_user(message, target_user_id, target_username=None):
    display_target = f"`{target_user_id}`{f' (@{telebot.util.escape(target_username)})' if target_username else ''}"
...
def handle_list_blocked(message):
    ...
    for user_id in sorted(list(blocked_users)):
        ...
        user_list_items.append(f"- `{user_id}`{f' (@{telebot.util.escape(username)})' if username else ''}")
